...
if __name__=='__main__':         
    ...
    #===========================按照最短距离选择路线===============================
    c = bfs_lenth(station_con,"中山北路","四川北路",search_strategy=sort_by_distance)
    print("临平路=>桂林路 dfs距离最短路线:",str(c))
    # 用bfs计算最短路程的线路，由于节点较多（344个），层次较深，所以运行起来特别慢，
    # 十几站的路线站跑了24小时还没出结果，测试用较短路线进行
    # 结果: 临平路=>桂林路 dfs距离最短路线: ['中山北路', '上海火车站', '汉中路', '曲阜路', '天潼路', '四川北路']
    ...
    # 大概需要2-3分钟
    中山北路=>四川北路 dfs最少换乘路线: 
    ['中山北路', '上海火车站', '汉中路', '新闸路', '人民广场', '黄陂南路', '陕西南路', '新天地', '老西门', '豫园', '南京东路', '天潼路', '四川北路']
    '''
    
    
    #Combine the previous two ideas, find a more suitable path.
    #相比较而言，我更倾向于选择 距离更短的线路，
    #测试中如果选择换乘数较少的路线，很可能走很长距离，耗时更长
    #测试时：换乘3次只要坐5站，如果换乘2次需要坐11站，换乘次数最少可能就不是最优选择
    #可以想到的其他路径选择方式：1按照估算时间寻找时间最短路线，2直接按照经过的站点最少选择路线
